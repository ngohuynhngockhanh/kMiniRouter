	...
	IN	UCHAR			Channel) 
{
	...
	if ((pAd->Mlme.OneSecPeriodicRound % 2 == 1) || (pAd->CommonCfg.bAntEvalEnable == TRUE))
#endif /* CONFIG_STA_SUPPORT */
	{
		/* patch for AsicSetRxAnt failed*/
		if (pAd->CommonCfg.bAntEvalEnable == FALSE)
			...
			OPSTATUS_TEST_FLAG(pAd, fOP_AP_STATUS_MEDIA_STATE_CONNECTED)) 
		{
			 ...
			 */
#endif /* RELEASE_EXCLUDE */
			if ((pAd->RxAnt.EvaluateStableCnt < 2) || (realavgrssi1 > (pAd->RxAnt.Pair1LastAvgRssi + 5)) || (realavgrssi1 < (pAd->RxAnt.Pair1LastAvgRssi - 5)) || (pAd->CommonCfg.bAntEvalEnable))
				AsicEvaluateRxAnt(pAd);

				pAd->RxAnt.Pair1LastAvgRssi = realavgrssi1;
		}
#ifndef MT7601  /* just fix at main antenna if it doesn't associate with AP */
	 	else
		{
