...
VOID STARxCollectEvalRssi(
	IN PRTMP_ADAPTER pAd,
	IN RXWI_STRUC *pRxWI,
	IN RXINFO_STRUC *pRxInfo)
{
	CHAR eval_ant = pAd->RxAnt.Pair1SecondaryRxAnt;
	CHAR select_ant = (pRxWI->RxWISNR1 & 0x80) >> 7;
	UCHAR phymode;
	SHORT rssi, avg_rssi;
	...
	if (eval_ant == select_ant)
	{
		if (pAd->CommonCfg.bTriggerTimer)
		{
			...
			/* calculate SNR */
			phymode = get_pkt_phymode_by_rxwi(pRxWI);
			
			if (phymode == MODE_CCK)
			{
				pAd->RxAnt.CCK_SNR[eval_ant] += (42 - ((pRxWI->RxWISNR0 * 3) >> 3));
				pAd->RxAnt.CCK_Cnt[eval_ant] ++;
			}
			else
			{
				pAd->RxAnt.OFDM_SNR[eval_ant] += ConvertToSnr(pAd, pRxWI->RxWISNR0);
		...
		STA_COLLECT_RX_ANTENNA_AVERAGE_RSSI(pAd, ConvertToRssi(pAd, (UCHAR) pRxWI->RxWISNR2, RSSI_0, pRxWI->RxWISNR1, pRxWI->RxWIBW), 0);	/* Note: RSSI2 not used on RT73 */
	...
	IN RX_BLK *pRxBlk)
{
	...
	do {
		...
		if ((pAd->CommonCfg.bSWRxAntDiversity) && (pAd->CommonCfg.RxAntDiversityCfg != ANT_DIVERSITY_DISABLE))
		{ 
			if ((pRxInfo->U2M)
				...
				&& (MAC_ADDR_EQUAL(&pAd->CommonCfg.Bssid, &pHeader->Addr2))))
			{
				...
				{
					STARxCollectEvalRssi(pAd, pRxWI, pRxInfo);	/* MT7601 SW based PPAD */
				}
				else
				{
#ifdef MT7601
					STA_COLLECT_RX_ANTENNA_AVERAGE_RSSI(pAd, ConvertToRssi(pAd, (CHAR) pRxWI->RxWISNR2, RSSI_0, pRxWI->RxWISNR1, pRxWI->RxWIBW), 0);
#else
					STA_COLLECT_RX_ANTENNA_AVERAGE_RSSI(pAd, ConvertToRssi(pAd, (UCHAR) pRxWI->RxWIRSSI0, RSSI_0, 0, 0), 0);	/* Note: RSSI2 not used on RT73 */
#endif /* MT7601 */
					pAd->StaCfg.NumOfAvgRssiSample++;
				}
			}
